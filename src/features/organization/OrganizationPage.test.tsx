...
const mocks = vi.hoisted(() => ({
  ...
  updateEmployeeOrganization: vi.fn(),
  createDepartment: vi.fn(),
  updateDepartment: vi.fn(),
}))

vi.mock('../auth/AuthContext', () => ({
  useCurrentRoles: () => ({ roles: mocks.roles }),
}))
...
const employee = (
  ...
  employeeId: id,
  employeeNumber: 'PRIVATE-NUMBER-' + id,
  employeeName: name,
  departmentId: 1,
  jobGradeId: level,
  jobGradeName: level === 1 ? '부장' : '신입사원',
  jobGradeLevel: level,
  hireDate: '2024-03-04',
  employeeType: 'GENERAL' as const,
...
describe('OrganizationPage', () => {
  ...
  beforeEach(() => {
    ...
    mocks.fetchOrganization.mockResolvedValue({
      departments: [
        {
          departmentId: 1,
          departmentCode: 'PRIVATE-CODE',
          departmentName: '개발부',
          parentDepartmentId: null,
          displayOrder: 0,
          children: [],
          employees: [
            employee(1, '김부장', 1, null),
            employee(2, '이사원', 5, 1),
          ],
        },
      ],
    ...
    mocks.fetchJobGrades.mockResolvedValue([
      {
        jobGradeId: 1,
        jobGradeName: '부장',
        jobGradeLevel: 1,
  ...
  async function render() {
    await act(async () => root.render(<OrganizationPage />))
  }
  ...
  it.each(
    ['EMPLOYEE', 'MANAGER', 'SYSTEM_ADMIN'] as RoleType[],
  )('hides every edit control from %s', async role => {
    mocks.roles = [role]
  ...
  it('allows HR to remove a manager and refreshes the chart after saving', async () => {
    ...
    expect(
      [...managerSelect.options].map(value => value.text),
    ).not.toContain('이사원 · 개발부 · 신입사원')

    await act(async () => {
      managerSelect.value = ''
      managerSelect.dispatchEvent(
    ...
    expect(mocks.updateEmployeeOrganization).toHaveBeenCalledWith(
      2,
      {
        departmentId: 1,
        jobGradeId: 5,
  ...
  function organizationWithStatus(
  ...
  function statusSelect() {
    return [
      ...container.querySelectorAll('dialog label'),
    ]
      .find(label =>
        label.textContent?.startsWith('재직 상태'),
      )!
  ...
  it.each([
    ...
    'saves %s to %s and reflects the refreshed leave badge',
    async (before, after, label) => {
      mocks.roles = ['HR_MANAGER']

      mocks.fetchOrganization
        .mockResolvedValueOnce(
          organizationWithStatus(before),
        )
        .mockResolvedValueOnce(
          organizationWithStatus(after),
        )

      await editEmployee()

      expect(statusSelect().value).toBe(before)
      expect(
      ...
      ).toContainEqual(['RETIRED', '퇴사'])

      expect(
        container
          .querySelector('[data-employee-id="2"]')
          ?.textContent?.includes('휴직'),
      ).toBe(before === 'LEAVE')
      ...
      await selectStatus(after)
      await submitEmployee()

      expect(
        mocks.updateEmployeeOrganization,
      ).toHaveBeenCalledWith(2, {
        departmentId: 1,
        jobGradeId: 5,
        managerEmployeeId: 1,
        employmentStatus: after,
      })
  ...
  it('cancels retirement without saving and confirms before refreshing away the employee', async () => {
    mocks.roles = ['HR_MANAGER']

    mocks.fetchOrganization
      .mockResolvedValueOnce(
        organizationWithStatus('EMPLOYED'),
      )
      .mockResolvedValueOnce(
        organizationWithStatus('RETIRED'),
      )

    await editEmployee()

    await selectStatus('RETIRED')
    await submitEmployee()

    expect(
      container.querySelector('dialog h2')?.textContent,
    ).toBe('퇴사 처리')

    expect(
      container.querySelector('dialog')?.textContent,
    ).toContain(
      '퇴사 처리 후 현재 조직도에서는 표시되지 않습니다.',
    )
    ...
    expect(statusSelect().value).toBe('RETIRED')
  ...
  it('cancels retirement with Escape without saving', async () => {
    mocks.roles = ['HR_MANAGER']
  ...
  it('blocks missing employment status instead of submitting undefined', async () => {
    mocks.roles = ['HR_MANAGER']
    ...
    Object.assign(
      response.departments[0].employees[1],
      {
        employmentStatus: undefined,
      },
    )

    mocks.fetchOrganization.mockResolvedValueOnce(
      response,
    )
  ...
  it.each([400, 403])(
    'keeps retirement errors visible for HTTP %s',
    async status => {
      mocks.roles = ['HR_MANAGER']

      mocks.updateEmployeeOrganization.mockRejectedValueOnce(
        new AppError(
          status,
          'ERROR',
          'Error',
  ...
  it('keeps headquarters visible but disabled only in employee editing and preserves the saved affiliation', async () => {
    ...
    const [department, grade, manager] =
  ...
  it('filters headquarters recursively, preserves search ancestors, and edits nested departments', async () => {
    const department = (
      ...
      employees:
        OrganizationDepartmentNode['employees'] = [],
    ): OrganizationDepartmentNode => ({
      departmentId: id,
  ...
  it('expands and collapses headquarters and limits team selection and search to that team', async () => {
    ...
    expect(
      container.querySelector(
        'section[aria-label="직속 상급자 조직도"] h2',
      )?.textContent,
    ).toBe('전체 조직도')
