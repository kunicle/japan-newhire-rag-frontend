...
export function mapNewHireErrorMessage(error: unknown): string {
  if (error instanceof AppError) {
    ...
    if (error.code === 'MANAGER_NOT_FOUND') return '선택한 직속 상급자를 사용할 수 없습니다. 조직 정보를 새로고침해 주세요.'
    if (error.code === 'MANAGER_GRADE_NOT_ALLOWED') return '직원보다 낮은 직급의 상급자를 지정할 수 없습니다.'
    if (error.code === 'SELF_MANAGER_NOT_ALLOWED' || error.code === 'MANAGER_CYCLE_NOT_ALLOWED') return '선택한 직속 상급자와 연결할 수 없습니다.'
