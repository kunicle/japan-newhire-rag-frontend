...
export function NewHireRegistrationPage() {
  ...
  return <div className={styles.page}>
    ...
    {result && <section className={styles.result} aria-labelledby="result-title" role="status"><div className={styles.resultHeader}><div><h2 id="result-title">신입사원 계정이 생성되었습니다.</h2><p>오늘 할 일과 교육 배정에 사용할 수 있습니다.</p></div><Badge variant="success">{result.accountStatus}</Badge></div><dl><div><dt>이름</dt><dd>{result.employeeName}</dd></div><div><dt>사번</dt><dd>{result.employeeNumber}</dd></div><div><dt>이메일</dt><dd>{result.email}</dd></div><div><dt>부서</dt><dd>{organizationDisplayLabel(result.departmentName)}</dd></div><div><dt>직급</dt><dd>{organizationDisplayLabel(result.jobGradeName)}</dd></div><div><dt>기본 역할</dt><dd>직원</dd></div></dl><Link className={styles.onboardingLink} to="/hr/onboarding">오늘 할 일 관리로 이동</Link></section>}
  </div>
}
